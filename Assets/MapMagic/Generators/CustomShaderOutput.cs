...
namespace MapMagic
{
	...
	public class CustomShaderOutput  : OutputGenerator
	{
		...
		public void DrawWarnings (Layout layout)
		{
			...
			if (MapMagic.instance.customTerrainMaterial == null || (!MapMagic.instance.customTerrainMaterial.shader.name.Contains(shaderName) && mode!=Mode.Custom))
			{
				...
				if (layout.Button("Fix",rect:layout.Inset(0.2f))) 
				{
					...
					#if CTS_PRESENT
					if (ctsProfile != null) CTS_ProfileToMaterial(ctsProfile, MapMagic.instance.customTerrainMaterial);
					#endif
				}
			}
		...
		public override void OnGUI (GeneratorsAsset gens)
		{
			...
			//layers
			layout.Par(5); layout.Foldout(ref guiLayers, mode==Mode.MegaSplat? "Clusters:" : "Layers:");
			if (guiLayers)
			{
		...
		public void OnLayerGUI (Layout layout, bool selected, int num)
		{
			...
			{
				layout.Inset(3);
				layout.Icon(textureList.clusters[baseLayers[num].index].previewTex, rect:layout.Inset(40), frame:true, alphaBlend:false);
		...
		static public void UpdateCustomShaderMaterials()
		{
			...
			if (CustomShaderOutput.mode == CustomShaderOutput.Mode.CTS && CustomShaderOutput.ctsProfile != null)
				CustomShaderOutput.CTS_ProfileToMaterial(CustomShaderOutput.ctsProfile, MapMagic.instance.customTerrainMaterial);
			#endif
			...
			foreach (Chunk chunk in MapMagic.instance.chunks.All())
			{
				...
				mat = new Material(MapMagic.instance.customTerrainMaterial);
		...
		#if CTS_PRESENT
		public static void CTS_ProfileToMaterial (CTS.CTSProfile m_profile, Material m_material)
		//this is the copy of CompleteTerrainShader.UpdateShader method that can work without terrain and use a custom material
		{
            //Global settings
            ...
            //ReAttachTextures();
			 #if UNITY_EDITOR
            //Profile path
            string profilePath = UnityEditor.AssetDatabase.GetAssetPath(m_profile);
            string arrayPath = System.IO.Path.GetDirectoryName(profilePath) + "/Arrays/";
            if (System.IO.Directory.Exists(arrayPath))
            {
                profilePath = arrayPath + System.IO.Path.GetFileName(profilePath);
            }
            ...
            {
                m_profile.AlbedosTextureArray = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2DArray>(profilePath + "_Albedos.asset");
            }
            ...
            if (m_profile.NormalsTextureArray == null)
            {
                ...
                m_profile.NormalsTextureArray = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2DArray>(profilePath + "_Normals.asset");
            }
